...
from DEC.pt_DEC.utils import To_cls, target_distribution
...
def train(dataset: torch.utils.data.Dataset,
    ...
    # kmeans : n_clusters = 형성할 클러스터의 수와 생성할 중심의 수입니다.
    #          n_init = k-평균 알고리즘이 다른 중심 시드로 실행되는 횟수입니다. 최종 결과는 관성 측면에서 n_init 연속 실행의 최상의 출력이 됩니다.
    ...
    for index, batch in enumerate(tqdm(data_iterator)):
    ...
    for epoch in trange(epochs):
        ...
        for index, batch in enumerate(tqdm(data_iterator)):
            ...
            print(f'index : {index}, loss : {loss_value}')
    ...
    for batch in dataloader:
        ...
        predicted_sm = F.softmax(predicted)

        predicted_sm = predicted_sm.tolist()
        for i in predicted_sm:
...
def predict(dataset: torch.utils.data.Dataset,
    ...
    for batch in tqdm(data_iterator):
        ...
        batch = batch.to(device)
        features.append(
            # To_cls(batch, bertmodel).detach().cpu()
    ...
    plt.title('BERT, Tsne, SK-means')
    ax.view_init(60, -60)
    plt.savefig("bert_1.png")
    plt.show()
    # if return_actual:
